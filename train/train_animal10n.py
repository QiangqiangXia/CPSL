...
parser.add_argument('--data_path', default='/root/autodl-tmp/animal10n', type=str, help='path to dataset')
...
def train(epoch,net,tch_net,optimizer,centers,labeled_trainloader,unlabeled_trainloader):
    ...
    for batch_idx, (inputs_x1, inputs_x2, inputs_x3, inputs_x4, labels_x, index, w_x) in enumerate(labeled_trainloader):      
        ...
        sys.stdout.write('\r')
        sys.stdout.write('Animal10N: | Epoch [%3d/%3d] Iter[%3d/%3d]\t Loss: %.2f'
...
def warmup(epoch,net,optimizer,dataloader):
    ...
    for batch_idx, (inputs, labels, _, _) in enumerate(dataloader):      
        ...
        sys.stdout.write('\r')
        sys.stdout.write('Animal10N | Epoch [%3d/%3d] Iter[%3d/%3d]\t CE-loss: %.4f'
                %(epoch, args.num_epochs, batch_idx+1, num_iter, loss.item()))
        sys.stdout.flush()
...
class CLoss(torch.nn.Module):
    ...
    def forward(self, outputs, labels):
        ...
        label_one_hot = torch.clamp(label_one_hot, min=1e-1, max=1.0) #min=1e-10forCifar100;min=1e-20forTiny;min=1e-1foranimal10N;min=1e-45forClothing1M
...
test_log=open('./checkpoint/animal10n_acc.txt','a')
loss_log=open('./checkpoint/animal10n_loss.txt','a')     
...
for epoch in range(start_epoch, args.num_epochs):
    ...
    if epoch<warm_up:       
        ...
        prob1, loss1, centers_eval1, paths1 = eval_train(net1, epoch, centers1)   
        prob2, loss2, centers_eval2, paths2  = eval_train(net2, epoch, centers2)          
        ...
        print('Train Net1')
        labeled_trainloader, unlabeled_trainloader = loader.run('train',pred=pred2,prob=prob2,paths=paths2) # co-divide             
        train(epoch,net1,tch_net1,optimizer1,centers1,labeled_trainloader,unlabeled_trainloader)
        
        print('\nTrain Net2')
        labeled_trainloader, unlabeled_trainloader = loader.run('train',pred=pred1,prob=prob1,paths=paths1) # co-divide
    ...
    if epoch == 19:
        filepath19 = os.path.join('./checkpoint', 'model19.pth.tar')
        save(filepath19,net1,net2,tch_net1,tch_net2,optimizer1,optimizer2,epoch,centers1,centers2,best_acc)
    if epoch == 39:
        filepath39 = os.path.join('./checkpoint', 'model39.pth.tar')
        save(filepath39,net1,net2,tch_net1,tch_net2,optimizer1,optimizer2,epoch,centers1,centers2,best_acc)
