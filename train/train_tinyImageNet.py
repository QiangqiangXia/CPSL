...
import dataloader_tiny as dataloader
...
def train(epoch,net,tch_net,optimizer,centers,labeled_trainloader,unlabeled_trainloader):
    ...
    for batch_idx, (inputs_x1, inputs_x2, inputs_x3, inputs_x4, labels_x, index, w_x) in enumerate(labeled_trainloader):      
        ...
        sys.stdout.write('\r')
        sys.stdout.write('Tiny: | Epoch [%3d/%3d] Iter[%3d/%3d]\t Loss: %.2f'
...
def warmup(epoch,net,optimizer,dataloader):
    ...
    for batch_idx, (inputs, labels, _) in enumerate(dataloader):      
        ...
        sys.stdout.write('\r')
        sys.stdout.write('Tiny | Epoch [%3d/%3d] Iter[%3d/%3d]\t CE-loss: %.4f'
                %(epoch, args.num_epochs, batch_idx+1, num_iter, loss.item()))
        sys.stdout.flush()
...
class CLoss(torch.nn.Module):
    ...
    def forward(self, outputs, labels):
        ...
        label_one_hot = torch.clamp(label_one_hot, min=1e-20, max=1.0) #min=1e-10forCifar100;min=1e-20forTiny;min=1e-45forClothing1M
...
loader = dataloader.tinyImagenet_dataloader(ratio=args.r,noise_mode=args.noise_mode,batch_size=args.batch_size,num_workers=5,\
    root=args.data_path,noise_file='%s/%.1f_%s.json'%(args.data_path,args.r,args.noise_mode))
...
for epoch in range(start_epoch, args.num_epochs):
    ...
    if epoch<warm_up:       
        ...
        print('Train Net1')
        labeled_trainloader, unlabeled_trainloader = loader.run('train',pred=pred2,prob=prob2) # co-divide             
        train(epoch,net1,tch_net1,optimizer1,centers1,labeled_trainloader,unlabeled_trainloader)
        
        print('\nTrain Net2')
        labeled_trainloader, unlabeled_trainloader = loader.run('train',pred=pred1,prob=prob1) # co-divide
    ...
    if epoch == 14:
        filepath14 = os.path.join('./checkpoint', 'model14.pth.tar')
        save(filepath14,net1,net2,tch_net1,tch_net2,optimizer1,optimizer2,epoch,centers1,centers2,best_acc)
    if epoch == 29:
