...
from augmentation_cifar import CIFAR10Policy
...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, r, noise_mode, root_dir, transform, mode, noise_file='', pred=[], probability=[], log='', test_form = None):
...
#                 f'选择的mask样本中正确选取的不干净标签数量{len(noise_index)}, 不正确选取的干净数量{len(pred_idx) - len(noise_index)}')
            return len(clean_index), (len(pred_idx1) - len(clean_index)), len(noise_index), len(pred_idx) - len(noise_index)
...
class cifar_dataloader():  
    def __init__(self, dataset, r, noise_mode, batch_size, num_workers, root_dir, log, noise_file=''):
        ...
        if self.dataset=='cifar10':
            self.transform = {
                "warmup": transform_cifar10,
                "unlabeled": [transform_cifar10, transform_cifar10, transform_strong_10, transform_strong_10],
                "labeled": [transform_cifar10, transform_cifar10, transform_strong_10, transform_strong_10]
            }
            ...
            self.transform = {
                "warmup": transform_cifar100,
                "unlabeled": [transform_cifar100, transform_cifar100, transform_strong_100, transform_strong_100],
                "labeled": [transform_cifar100, transform_cifar100, transform_strong_100, transform_strong_100]
            }
            self.transform_test = transforms.Compose([
                    transforms.ToTensor(),
                    transforms.Normalize((0.507, 0.487, 0.441), (0.267, 0.256, 0.276)),
    ...
    def run(self,mode,pred=[],prob=[]):
        if mode=='warmup':
            all_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transform["warmup"], mode="all",noise_file=self.noise_file)                
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transform["labeled"], mode="labeled", noise_file=self.noise_file, pred=pred, probability=prob,log=self.log)              
            ...
            unlabeled_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=self.transform["unlabeled"], mode="unlabeled", noise_file=self.noise_file, pred=pred, log=self.log)
