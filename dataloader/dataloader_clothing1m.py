...
class clothing_dataset(Dataset): 
    def __init__(self, root, transform, mode, num_samples=0, pred=[], probability=[], paths=[], num_class=14): 
        ...
        if mode == 'all':
            ...
            with open('%s/annotations/clean_train_key_list.txt'%self.root,'r') as f:
                lines = f.read().splitlines()
                for l in lines:
                    img_path = '%s/'%self.root+l[7:]
                    train_imgs.append(img_path)
...
class clothing_dataloader:
    ...
    def run(self,mode,pred=[],prob=[],paths=[]):        
        if mode=='warmup':
            warmup_dataset = clothing_dataset(self.root,transform=self.transform_train["warmup"], mode='all',num_samples=self.num_batches*self.batch_size)
            ...
            labeled_dataset = clothing_dataset(self.root,transform=self.transform_train["labeled"], mode='labeled',pred=pred, probability=prob,paths=paths)
            labeled_loader = DataLoader(
                dataset=labeled_dataset, 
                batch_size=self.batch_size,
                shuffle=True,
                num_workers=self.num_workers)           
            unlabeled_dataset = clothing_dataset(self.root,transform=self.transform_train["unlabeled"], mode='unlabeled',pred=pred, probability=prob,paths=paths)
